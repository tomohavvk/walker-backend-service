...
trait DeviceRepository[D[_]] {
  def upsert(entity:     DeviceEntity): D[Int]
  def findById(deviceId: DeviceId): D[Option[DeviceEntity]]
}

class DoobieDeviceRepository[D[_]](implicit D: LiftConnectionIO[D, AppError])
    extends DeviceRepository[D]
    with DeviceQueries {

  override def upsert(entity: DeviceEntity): D[Int] =
